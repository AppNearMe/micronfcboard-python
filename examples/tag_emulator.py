...
if not board.connected:
    #Set NDEF message for emulation
    board.ndefRecords = [SmartPosterRecord([URIRecord("http://www.micronfcboard.com/"), TextRecord("MicroNFCBoard", "en")])]
    print("Start polling")
    board.startPolling(False, True, False)

while board.polling:
    sleep(0.1)

if board.connected and board.type4Emulator:
    print("Emulating type 4 tag")
else:
...
while board.connected or board.polling:
